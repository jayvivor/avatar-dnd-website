  ...
  for row in form_range:
    ...
    if row[5].value:  # F corresponds to 5
      form_dict = {
        ...
        "has_higher_level_bonus": False,
        "concentration": bool(row[8].value),  # I corresponds to 8
        "target": row[9].value,  # J corresponds to 9
        "casting_speed": models.DndCastingSpeed.objects.get(name=row[10].value),  # K corresponds to 10
        "duration": row[11].value,  # L corresponds to 11
        "range": row[12].value,  # M corresponds to 12
        "components": models.DndComponent.objects.get_or_create(name=row[13].value),  # N corresponds to 13
        "saving_throw": models.DndAbility.objects.get(name=row[15].value),  # P corresponds to 15
        "classes": models.DndClass.objects.filter(parse_dnd_classes(name=row[16].value)).all(),  # Q corresponds to 16
        "costs_slot": bool(row[17].value),  # R corresponds to 17
        "special_reqs": row[18].value,  # S corresponds to 18
      }

      # Retrieve discipline by ID

      form_dict["discipline"] = models.DndDiscipline.objects.get(name=form_dict["discipline"], element=form_dict["element"]).id
      ...
      if row[7].value:  # H corresponds to 7
          form_dict.update({"has_higher_level_bonus": True, "higher_levels": row[7].value})  # H corresponds to 7
